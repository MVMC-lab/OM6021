...
為了能夠在Matlab內即可以直接與ASA單板電腦進行大批實測及模擬數據。ASA開發團隊提供了Matlab 文字人機巨集函式庫。巨集函式庫提供了包括：可用於開啟關閉UART通訊的，REMO_open(), REMO_close()、可用與ASA單板電腦協調對話的scanf(),printf()、用於設定通訊輪詢次數上限的REMO_set()、用於與ASA單板交換矩陣內容的 REMO_put(), REMO_get()、以及用於與ASA單板交換結構內容的 REMOForm_put(), REMOForm_get()。 由以下Matlab人機函式與ASA人機函式對應功能方塊圖所顯示。
...
- Matlab REMOForm_get() 呼應ASA單板的HMIForm_put()：當雙方完成同步，並協調要由ASA單板送結構內容到Matlab上時，ASA單板程式應呼叫HMIForm_put() 將某結構內容送出。而Matlab則呼叫REMOForm_get()輪詢讀取送來內容。
ASA單板的HMI_get() 呼應Matlab REMO_put()：當雙方完成同步，並協調要由Matlab送矩陣內容到ASA單板上時，Matlab應呼叫REMO_put() 將某矩陣內容送出。而ASA單板程式則呼叫HMI_get() 輪詢讀取送來內容。
ASA單板的HMIForm_get() 呼應Matlab REMOForm_put()：當雙方完成同步，並協調要由Matlab送結構內容到ASA單板上時，Matlab應呼叫REMOForm_put() 將某矩陣內容送出。而ASA單板程式則呼叫HMIForm_get() 輪詢讀取送來內容。
...
- 簡介：呼叫本巨集函式由Port 指定之UART通訊埠讀回FormatString 所指定型態之結構內容。
- 輸入變數：
 - Port : 由Port變數集合所代表之已開啟UART通訊埠。
- 回傳結果：
...
##### Data_struct
Data_struct在MATLAB裏的結構各欄位之名稱會依欄位次序以及形態命名，同前述例子，
若結構名稱Data_struct則
...
- 簡介：呼叫本巨集函式由Port 指定之UART通訊埠送出FormatString 所指定型態之結構內容。
- 輸入變數：
 - Port : 由Port變數集合所代表之已開啟UART通訊埠。
